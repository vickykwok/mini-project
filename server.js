...
const handle_Find = (res, criteria) => {
    ...
    client.connect((err) => {
        ...
        findDocument(db, criteria, (docs) => {
            ...
            res.status(200).render('find',{nrestaurant: docs.length, restaurant: docs,restaurant_id:docs[0]});

        });
    });
}
//display identical restaurant from mongodb
const handle_Details = (res, criteria) => {
    ...
    client.connect((err) => {
        ...
        findDocument(db, DOCID, (docs) => {  
            client.close();
            console.log("Closed DB connection");
            res.status(200).render('details', {restaurant_id:docs[0]});
        });
    });
}
//remove identical restaurant from list/mongodb
...
const handle_Rate = (res, criteria) => {
    ...
    client.connect((err) => {
        ...
        db.collection('restaurant').updateDocument(DOCID, updateDoc, (results) => {
            res.status(200).render('rate', {restaurant_id:docs[0]});
...
//edit identical restaurant information
        ...
        if (req.files.filetoupload.size > 0) {
            fs.readFile(req.files.filetoupload.path, (err,data) => {
                ...
                updateDocument(DOCID, updateDoc, (results) => {
                    res.status(200).render('update', {restaurant_id: updateDoc[0]})
                });
            });
        } else {
            updateDocument(DOCID, updateDoc, (results) => {
...
/* READ-get get the particular document in json*/
